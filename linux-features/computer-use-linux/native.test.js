...
readline.createInterface({ input: process.stdin }).on('line', line => {
  ...
  if(a.text === 'oversized') return process.stdout.write(Buffer.alloc(8*1024*1024+1, 120));
  if(a.text === 'ambiguous') return process.stdout.write(JSON.stringify({jsonrpc:'2.0',id:r.id,result:{},error:{code:-1}})+'\\n');
  if(a.text === 'fragmented') {
    const wire=JSON.stringify({jsonrpc:'2.0',id:r.id,result:{structuredContent:{ok:true},content:[]}})+'\\n';
    process.stdout.write(wire.slice(0,17));
    return setTimeout(()=>process.stdout.write(wire.slice(17)),10);
  }
...
test('MCP initialization, exact window IDs, targeted inputs and rejected desktop inputs', async t => {
  ...
  await assert.rejects(service.handleRpc({method:'press_key',params:{key:'ESC'}}), /app id is required/);
});
...
test('backend errors surface and an exited backend is never silently replayed or restarted', async t => {
  const service = await fixture(t);
  await assert.rejects(service.handleRpc({method:'type_text',app:'editor',params:{text:'tool-error'}}), /denied by backend/);
  await assert.rejects(service.handleRpc({method:'type_text',app:'editor',params:{text:'action-error'}}), /focus changed/);
  await assert.rejects(service.handleRpc({method:'type_text',app:'editor',params:{text:'exit'}}), /exited/);
...
test('missing targets, shutdown and malformed protocol fail without replay', async t => {
  ...
  await assert.rejects(service.handleRpc({method:'type_text',app:'editor',params:{text:'malformed'}}), /invalid JSON/);
...
test('timed-out input poisons the transport rather than allowing replay', async t => {
  const service = await fixture(t);
  await assert.rejects(service.handleRpc({method:'type_text',app:'editor',params:{text:'hang'}}), /timed out/);
...
// Lab safety regressions; no personal desktop, network or credential access.
test('Lab input bounds and target requirements fail before backend launch', async t => {
  const { createNativeService } = await import('./native-service.mjs');
  const service = createNativeService({command:'/nonexistent/native-backend'});
  t.after(()=>service.shutdown());
  for (const method of ['screenshot','get_app_state','click','scroll','press_key','type_text']) {
    await assert.rejects(service.handleRpc({method}), /app id is required/);
  }
  for (const params of [{x:-1,y:2},{x:1.5,y:2},{x:1,y:2,relative:false},{x:1,y:2,button:'unknown'},{x:1,y:2,click_count:4}]) {
    await assert.rejects(service.handleRpc({method:'click',app:'editor',params}), /Invalid native/);
  }
  for (const text of ['', 'x'.repeat(16385), null, 42]) {
    await assert.rejects(service.handleRpc({method:'type_text',app:'editor',params:{text}}), /Invalid native/);
  }
});

test('Lab fragmented JSON-RPC is reconstructed without replay', async t => {
  const service=await fixture(t);
  assert.deepEqual(await service.handleRpc({method:'type_text',app:'editor',params:{text:'fragmented'}}),{ok:true});
  assert.equal((await service.readCalls()).length,1);
});

for (const [text, reason] of [['oversized', /frame limit/], ['ambiguous', /ambiguous JSON-RPC/]]) {
  test(`Lab ${text} transport fails closed and cannot replay input`, async t => {
    const service=await fixture(t);
    await assert.rejects(service.handleRpc({method:'type_text',app:'editor',params:{text}}),reason);
    await assert.rejects(service.handleRpc({method:'list_apps'}),reason);
    assert.equal((await service.readCalls()).length,1);
  });
}
