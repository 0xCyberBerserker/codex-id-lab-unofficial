...
function codexLinuxAppshotBackendPath(){let fs=codexLinuxAppshotRequire(\`node:fs\`),path=codexLinuxAppshotRequire(\`node:path\`),resources=process.resourcesPath,override=process.env.CODEX_LAB_APPROVED_COMPUTER_USE_BACKEND,candidate=override||(resources&&path.join(resources,\`plugins\`,\`openai-bundled\`,\`plugins\`,\`unified-computer-use\`,\`bin\`,\`codex-computer-use-linux\`));if(typeof candidate!=\`string\`||!path.isAbsolute(candidate))return null;try{let stat=fs.lstatSync(candidate);if(!stat.isFile()||(stat.mode&18)!==0)return null;fs.accessSync(candidate,fs.constants.X_OK);if(override){let expected=process.env.CODEX_LAB_APPROVED_COMPUTER_USE_BACKEND_SHA256;if(!/^[0-9a-f]{64}$/.test(expected||\`\`)||codexLinuxAppshotRequire(\`node:crypto\`).createHash(\`sha256\`).update(fs.readFileSync(candidate)).digest(\`hex\`)!==expected)return null}return candidate}catch{return null}}
