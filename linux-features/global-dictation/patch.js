...
// Pinned source and local Wayland-only safety changes: third-party/upstream.lock.json.
"use strict";
...
function codexLinuxGlobalDictationNativePath(name) {
  if (name !== "codex-global-dictation-linux" || !process.resourcesPath) return null;
  const fs = require("node:fs"), path = require("node:path");
  const candidate = path.join(process.resourcesPath, "native", name);
  try {
...
function codexLinuxGlobalDictationPasteX11() {
  return Promise.reject(new Error("X11 global dictation is not enabled in this Lab candidate."));
}
...
function codexLinuxGlobalDictationReleaseWatcher() {
  // Release events are supplied only by the consent-based Wayland portal helper.
  return null;
}
  ...
  try {
    ...
    let patched = replaceUnique(
      ...
      (original, _functionName, _bareTest, _bareSupport, _bareRegister, callbacksVar) => {
        ...
        return withUnavailable.replace(
          ";if(",
          `;if(process.platform===\`linux\`)return codexLinuxGlobalDictationUsesWayland()?codexLinuxGlobalDictationPortalRegistration(e,${callbacksVar}):null;if(`,
        );
      },
    ...
    patched = replaceUnique(
      ...
      (_original, functionName) =>
        "function " + functionName + "(){return process.platform===`darwin`||process.platform===`win32`||(process.platform===`linux`&&codexLinuxGlobalDictationUsesWayland())}",
