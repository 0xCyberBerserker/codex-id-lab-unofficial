...
// See third-party/upstream.lock.json for source provenance and local changes.
use std::{
    collections::HashMap,
    io::Write,
...
async fn paste_through_portal(
    ...
    for (keysym, state) in [
        ...
        if let Err(error) = result {
            // Best-effort release after an interrupted chord; never leave modifiers held.
            for release in [KEYSYM_V, KEYSYM_CONTROL_L] {
                let _: Result<(), zbus::Error> = proxy
                    .call(
                        "NotifyKeyboardKeysym",
                        &(session, &options, release, KEY_RELEASED),
                    )
                    .await;
            }
            return Err(error).context("RemoteDesktop keyboard input failed");
        }
    }
    Ok(())
}
...
mod tests {
    ...
    impl FakeRemoteDesktop {
        ...
        async fn notify_keyboard_keysym(
            ...
            if self.fail_paste_press && keysym == KEYSYM_V && state == KEY_PRESSED {
                return Err(zbus::fdo::Error::Failed(
                    "injected keyboard failure".to_string(),
                ));
            }
            Ok(())
        }
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn keyboard_failure_releases_the_interrupted_paste_chord() {
        let bus = TestBus::start();
        let remote = FakeRemoteDesktop {
            fail_paste_press: true,
            ..FakeRemoteDesktop::default()
        };
        let observed = remote.key_events.clone();
        let _server = Builder::address(bus.address.as_str())
            .unwrap()
            .name(PORTAL_SERVICE)
            .unwrap()
        ...
        let client = Builder::address(bus.address.as_str())
            ...
            .await
            .unwrap();
        assert!(paste_through_portal(&client, &mut None).await.is_err());
        assert_eq!(
            &observed.lock().unwrap()[2..],
            &[(KEYSYM_V, KEY_RELEASED), (KEYSYM_CONTROL_L, KEY_RELEASED)]
        );
    }
