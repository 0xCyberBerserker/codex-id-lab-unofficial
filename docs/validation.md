...
The separate final AppImage experiment executes the real signed runtime's extract-and-run path and bundled AppRun, with the same isolation. Its later, visually inspected capture reaches the real ChatGPT sign-in page: anonymous renderer/login-screen PASS. Authentication, FUSE, voice, microphone and cross-distribution acceptance remain NOT_RUN. This does not upgrade the CI window-only smoke into an authenticated gate.
...
La prueba separada del AppImage final ejecuta extract-and-run del runtime firmado y AppRun reales, con el mismo aislamiento. Su captura posterior, inspeccionada visualmente, llega a la página real de login ChatGPT: renderer/pantalla de login anónimos PASS. Autenticación, FUSE, voz, micrófono y otras distribuciones siguen NOT_RUN. Esto no convierte el smoke de ventana de CI en un gate autenticado.
